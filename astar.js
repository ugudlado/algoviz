...
(() => {
  ...
  function runAStar() {
    if (!startPos || !endPos) {
      updateInfo("Please set both a start and end point before running A*.");
      return;
    }
  ...
  function renderSnapshot(stepIdx) {
    ...
    if (compareMode && bfsSnapshots.length > 0) {
      ...
      const astarStatsEl = document.getElementById("astar-panel-stats-astar");
      const bfsStatsEl = document.getElementById("astar-panel-stats-bfs");
      if (astarStatsEl) {
        astarStatsEl.textContent = "Explored: " + explicitClosedSize + " cells";
      }
      if (bfsStatsEl) {
        bfsStatsEl.textContent = "Explored: " + explicitBfsExplored + " cells";
      }
    }

    // Info
    if (stepIdx === snapshots.length - 1 && astarResult && astarResult.path) {
      updateInfo(
        "Path found! Length: " +
          astarResult.pathLength +
          " steps. A* explored " +
  ...
  function renderSnapshotOnGrid(gridId, snap, result) {
    ...
    if (result && result.path && currentStep === snapshots.length - 1) {
  ...
  function renderOpenSetList(listId, snap) {
    ...
    items.forEach((coord) => {
      ...
      if (cost) {
        item.textContent =
          "(" + coord[0] + "," + coord[1] + ") f=" + Math.round(cost.f);
      } else {
  ...
  function updateStats(sidebarId, step, openSize, closedSize, result) {
    ...
    if (pathEl) {
      pathEl.textContent =
        result && result.pathLength >= 0 ? String(result.pathLength) : "-";
    }
  }
  ...
  function setHeuristic(h) {
    heuristic = h;
    btnManhattan.classList.toggle("astar-active-heuristic", h === "manhattan");
    btnEuclidean.classList.toggle("astar-active-heuristic", h === "euclidean");
  }
