...
(() => {
  ...
  function initGrid() {
    ...
    resetQueueSidebar();
    infoEl.innerHTML =
      "Click cells to place walls, then set start &amp; end points and click <strong>Run BFS</strong>.";
  }
  ...
  function runBFS() {
    if (!startPos || !endPos) {
      infoEl.textContent =
        "Please set both a start and end point before running BFS.";
      return;
    }
  ...
  function resetQueueSidebar() {
    queueList.innerHTML =
      '<div class="bfs-queue-empty">Run BFS to see the queue</div>';
