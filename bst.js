(() => {
  ...
  function renderTree(visitedValues, currentNode) {
    ...
    for (let i = 0; i < layout.nodes.length; i++) {
      ...
      if (isVisited || isCurrentNode) {
        const badgeIdx = isCurrentNode ? visitedValues.length : visitIndex + 1;
  ...
  function showResult() {
    ...
    resultEl.textContent =
      "Traversal complete! Order: [" + traversalResult.result.join(", ") + "]";
  ...
  function insertValue() {
    ...
    updateStats();
    infoEl.textContent =
      "Inserted " + val + ". Tree has " + BSTAlgorithm.size(tree) + " node(s).";
    valueInput.value = "";
    valueInput.focus();
  }
