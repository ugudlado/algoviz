(() => {
  ...
  function renderStep(step) {
    ...
    if (
      step.type === "sort" &&
      scanResult &&
      scanResult.sortedIndices.length > 1
    ) {
  ...
  function generateRandom() {
    ...
    for (let i = 0; i < count; i++) {
      points.push({
        ...
        y:
          CANVAS_PADDING + Math.random() * (canvas.height - 2 * CANVAS_PADDING),
      });
    }
    ...
    updateStats();
    infoEl.textContent =
      count + " random points placed. Click Visualize to run Graham Scan.";
  }
  ...
  function clearPoints() {
    ...
    updateStats();
    infoEl.textContent =
      "Click on the canvas to place points, or use Random to generate them.";
  }
  ...
  function visualize() {
    ...
    if (points.length === 0) {
      infoEl.textContent =
        "Error: Place at least one point on the canvas or click Random.";
      return;
    }
  ...
  canvas.addEventListener("click", (e) => {
    ...
    if (points.length >= 100) {
      infoEl.textContent =
        "Error: Maximum 100 points reached. Clear to start over.";
      return;
    }
    ...
    infoEl.textContent =
      ...
      " point" +
      (points.length === 1 ? "" : "s") +
      " placed. Click Visualize to run Graham Scan.";
    updateStats();
  });
