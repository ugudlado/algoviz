...
(() => {
  ...
  function render() {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (node._dist !== undefined) {
        ...
        distLabel.textContent =
          node._dist === Infinity ? "\u221E" : String(node._dist);
      ...
      el.addEventListener("mousedown", (e) => {
        ...
        if (mode === "SOURCE") {
          sourceNode = node.id;
          infoEl.textContent =
            "Source set to " + node.id + ". Click Run Dijkstra.";
          render();
          e.stopPropagation();
          return;
        }
    ...
    // Edge mode cursor
    graphContainer.classList.toggle(
      "dijk-edge-mode",
      mode === "EDGE" || mode === "SOURCE",
    );
  }
  ...
  function handleEdgeClick(nodeId) {
    if (edgeStart === null) {
      edgeStart = nodeId;
      infoEl.textContent =
        "Now click the destination node for the edge from " + nodeId + ".";
      render();
    } else {
      if (edgeStart === nodeId) {
      ...
      if (hasEdge(edgeStart, nodeId)) {
        infoEl.textContent =
          "Edge from " + edgeStart + " to " + nodeId + " already exists.";
        edgeStart = null;
        render();
        return;
      }
  ...
  graphContainer.addEventListener("mousedown", (e) => {
    ...
    if (mode === "NODE") {
      ...
      infoEl.textContent =
        "Node " + id + " added. Add more or switch to Edge mode.";
      render();
    }
  });
  ...
  function renderPQ(pq) {
    ...
    for (let i = 0; i < pq.length && i < 30; i++) {
      ...
      distSpan.textContent =
        pq[i].distance === Infinity ? "\u221E" : String(pq[i].distance);
  ...
  function setMode(newMode) {
    ...
    if (mode === "NODE") {
      infoEl.textContent =
        "Click the canvas to add nodes (max " + MAX_NODES + ").";
    } else if (mode === "EDGE") {
      infoEl.textContent =
