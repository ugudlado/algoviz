(() => {
  ...
  function positionCar(floor, animate) {
    ...
    if (animate) {
      elevatorCar.style.transition =
        "bottom " + getAnimDuration() + "ms ease-in-out";
    } else {
  ...
  function updateFloorHighlights() {
    ...
    for (let i = 0; i <= maxFloor; i++) {
      if (floorElements[i]) {
        floorElements[i].classList.remove(
          "elev-current-floor",
          "elev-serviced",
        );
      }
  ...
  function updateInfo() {
    if (stepIdx < 0) {
      infoEl.innerHTML =
        "Click <strong>Play</strong> or <strong>Step &rarr;</strong> to start the elevator.";
      return;
    }
    const step = steps[stepIdx];
    ...
    infoEl.textContent =
      ...
      step.target +
      " (distance: " +
      Math.abs(step.target - step.position) +
      ", total: " +
      step.distanceSoFar +
      ")";
  }
  ...
  function showResult() {
    ...
    resultEl.textContent =
      "Total distance: " +
      solveResult.totalDistance +
      " | Requests serviced: " +
      solveResult.order.length;
  ...
  function visualize() {
    ...
    // Validate
    const invalidReqs = input.requests.filter(
      (r) => r < 0 || r > input.maxFloor,
    );
    ...
    if (input.startPos < 0 || input.startPos > input.maxFloor) {
      infoEl.textContent =
        "Error: Start position must be between 0 and " + input.maxFloor;
      return;
    }
  ...
  maxFloorInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") visualize();
  });
