(() => {
  ...
  function renderEncodingTable(encodingTable) {
    ...
    keys.sort(
      (a, b) =>
        encodingTable[a].length - encodingTable[b].length || a.localeCompare(b),
    );
  ...
  function showCompression() {
    ...
    compOriginal.textContent =
      text.length + " chars (" + originalBits + " bits)";
  ...
  function drawEdges(lay, highlightIds) {
    ...
    if (lay.left) {
      drawEdge(
        ...
        lay.left.y,
        "0",
        highlightIds.has(lay.left.node.id),
      );
    ...
    if (lay.right) {
      drawEdge(
        ...
        lay.right.y,
        "1",
        highlightIds.has(lay.right.node.id),
      );
  ...
  function drawNodes(lay, highlightIds) {
    ...
    if (isLeaf) {
      ...
      // Character label
      const charText = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "text",
      );
    ...
    const freqText = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "text",
    );
    freqText.setAttribute("x", lay.x);
    freqText.setAttribute("y", lay.y + (isLeaf ? 12 : 5));
    freqText.setAttribute(
      "class",
      isLeaf ? "huff-node-freq" : "huff-node-text",
    );
  ...
  function renderQueueAsTree(queue) {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      charText.textContent =
        node.char !== null ? (node.char === " " ? "SP" : node.char) : "*";
      treeSvg.appendChild(charText);
