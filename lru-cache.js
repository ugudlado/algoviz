(() => {
  ...
  function renderFromCache(highlightKey, highlightType) {
    renderList(LRUCacheAlgorithm.getOrder(cache), highlightKey, highlightType);
    renderMap(
  ...
  function renderList(order, highlightKey, highlightType, evictedInfo) {
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      if (entry.key === highlightKey) {
        if (highlightType === "promoted")
          nodeEl.classList.add("lru-node-promoted");
  ...
  function updateStats() {
    ...
    stepStat.textContent = presetMode
  ...
  function addLogEntry(step) {
    ...
    if (step.type === "get" && step.hit)
      entry.classList.add("lru-log-entry-hit");
    else if (step.type === "get") entry.classList.add("lru-log-entry-miss");
    else if (step.action === "evict")
      entry.classList.add("lru-log-entry-evict");
    else if (step.action === "update")
      entry.classList.add("lru-log-entry-update");
  ...
  function handleGet() {
    ...
    addLogEntry(step);
    renderFromCache(key, step.hit ? "promoted" : "miss");
    infoEl.textContent = step.explanation;
  }
