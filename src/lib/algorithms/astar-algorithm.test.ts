...
describe("astar algorithm", () => {
  ...
  const AStarAlgorithm =
    require("./astar-algorithm.ts").default || require("./astar-algorithm.ts");
  ...
  it(() => {
    ...
    assert(
      result.path.length === 9,
      "Shortest path on 5x5 from (0,0) to (4,4) is 9 cells (8 steps)",
    );
    assert(
      result.path[0][0] === 0 && result.path[0][1] === 0,
      "Path starts at (0,0)",
    );
    assert(
      result.path[8][0] === 4 && result.path[8][1] === 4,
      "Path ends at (4,4)",
    );
  ...
  it(() => {
    ...
    assert(
      result.path.length > 9,
      "Zigzag path is longer than direct 9-cell path",
    );
    ...
    for (var i = 0; i < result.path.length; i++) {
      ...
      for (var w = 0; w < walls.length; w++) {
        if (
          result.path[i][0] === walls[w][0] &&
          result.path[i][1] === walls[w][1]
        ) {
          isWall = true;
        }
      }
  ...
  it(() => {
    ...
    assert(
      result.path[0][0] === 2 && result.path[0][1] === 2,
      "Path is just the start/end cell",
    );
  ...
  it(() => {
    ...
    var diag = h(0, 0, 1, 1);
    assert(
      Math.abs(diag - Math.SQRT2) < 0.001,
      "Euclidean (0,0) to (1,1) = sqrt(2)",
    );
  ...
  it(() => {
    ...
    for (var j = 0; j < lastSnap.closedSet.length; j++) {
      ...
      assert(
        !openKeys[ck],
        "Closed set node " + ck + " should not be in open set",
      );
    }
  ...
  it(() => {
    ...
    assert(
      result.explored < bfsResult.explored,
      "A* explored (" +
        result.explored +
  ...
  it(() => {
    ...
    assert(
      Array.isArray(snap.closedSet),
      "BFS snapshot has closedSet (visited)",
    );
