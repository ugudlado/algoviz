...
describe("bloom filter algorithm", () => {
  ...
  const BloomFilterAlgorithm =
    require("./bloom-filter-algorithm.ts").default ||
    require("./bloom-filter-algorithm.ts");
  // --- createFilter ---
  it(() => {
    ...
    assertEqual(
      Object.keys(filter.insertedWords).length,
      0,
      "no inserted words",
    );
  ...
  it(() => {
    ...
    assert(
      typeof h1 === "number" && typeof h2 === "number",
      "both are numbers",
    );
  ...
  it(() => {
    ...
    assertEqual(
      result.result,
      "true-negative",
      "empty filter always true-negative",
    );
  ...
  it(() => {
    ...
    if (fill.percentage === 100) {
      ...
      assertEqual(
        result.isKnownInserted,
        false,
        "word was not actually inserted",
      );
    } else {
  ...
  it(() => {
    ...
    assertEqual(
      result.result,
      "false-positive",
      "false positive when all bits set",
    );
  ...
  it(() => {
    ...
    assertEqual(
      q.result,
      "true-negative",
      "query before insert is true-negative",
    );
