...
function runTests({ assert, assertEqual }) {
  ...
  test("no path from S to T: max flow = 0", function () {
    var graph = { S: { A: 10 }, A: {}, T: {} };
    var result = FordFulkersonAlgorithm.maxFlow(graph, "S", "T");
    assertEqual(result.flow, 0, "max flow should be 0 when no path");
  });

  test("parallel paths: max flow = sum of capacities", function () {
  ...
  test("single edge bottleneck: max flow limited by smallest cap", function () {
    var graph = {
      S: { A: 100 },
      A: { B: 1 },
      B: { T: 100 },
  ...
  test("classic 6-node preset: max flow = 23", function () {
  ...
  test("bipartite preset: max flow = 3", function () {
  ...
  test("simple preset: max flow = 5", function () {
  ...
  test("min-cut edges are correctly identified: source on S-side, sink on T-side", function () {
    var graph = {
      S: { A: 2, B: 2 },
      A: { T: 2 },
      B: { T: 2 },
  ...
  test("min-cut capacity equals max flow (max-flow min-cut theorem)", function () {
  ...
  test("steps array is non-empty", function () {
    var graph = { S: { T: 5 }, T: {} };
    var result = FordFulkersonAlgorithm.maxFlow(graph, "S", "T");
  ...
  test("last step is always type 'done'", function () {
  ...
  test("steps contain bfs-path-found when path exists", function () {
    var graph = { S: { T: 5 }, T: {} };
    var result = FordFulkersonAlgorithm.maxFlow(graph, "S", "T");
  ...
  test("steps contain augment steps when flow is pushed", function () {
    var graph = { S: { T: 5 }, T: {} };
    var result = FordFulkersonAlgorithm.maxFlow(graph, "S", "T");
  ...
  test("steps have required fields", function () {
    var graph = { S: { T: 3 }, T: {} };
    var result = FordFulkersonAlgorithm.maxFlow(graph, "S", "T");
  ...
  test("no path: steps contain bfs-no-path step", function () {
    var graph = { S: { A: 10 }, A: {}, T: {} };
    var result = FordFulkersonAlgorithm.maxFlow(graph, "S", "T");
  ...
  test("totalFlow in augment steps is monotonically increasing", function () {
    var graph = {
      S: { A: 3, B: 2 },
      A: { T: 3 },
      B: { T: 2 },
  ...
  test("buildResidual creates reverse edges with 0 capacity", function () {
    var graph = { S: { T: 5 }, T: {} };
