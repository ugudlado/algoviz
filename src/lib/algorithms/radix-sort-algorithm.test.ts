...
const RadixSortAlgorithm =
  require("./radix-sort-algorithm.ts").default ||
  require("./radix-sort-algorithm.ts");
describe("radix sort algorithm", () => {
  ...
  const referenceSort = (arr) => {
    return arr.slice().sort(function (a, b) {
      return a - b;
    });
  };

  it("sorts numbers and records expected step metadata", () => {
    ...
    assert(
      arraysEqual(r4.sortedArray, [7, 7, 7, 7]),
      "all duplicates unchanged",
    );
    ...
    assert(
      typeof s0.highlightIdx === "number",
      "step has numeric highlightIdx",
    );
    ...
    assert(
      arraysEqual(r14.sortedArray, [10, 20]),
      "two-element reverse sorted",
    );

    var r15 = RadixSortAlgorithm.sort([10, 20]);
    assert(
      arraysEqual(r15.sortedArray, [10, 20]),
      "two-element already sorted",
    );
  });
});
