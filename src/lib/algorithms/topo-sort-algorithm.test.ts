...
describe("topo sort algorithm", () => {
  ...
  const TopoSortAlgorithm =
    require("./topo-sort-algorithm.ts").default ||
    require("./topo-sort-algorithm.ts");
  // --- Helper: verify topological order validity ---
  const isValidTopoOrder = (nodes, edges, order) => {
  ...
  it(() => {
    ...
    assert(
      ...
      "At least one snapshot for single node",
    );
  }, "Single node graph");
  ...
  it(() => {
    ...
    assert(
      ...
      "Course prerequisites preset has no cycle",
    );
    assert(result.order.length === preset.nodes.length, "All nodes ordered");
    assert(
      isValidTopoOrder(preset.nodes, preset.edges, result.order),
  ...
  it(() => {
    ...
    assertEqual(
      result.order,
      ["A", "B"],
      "Correct order despite duplicate edges",
    );
