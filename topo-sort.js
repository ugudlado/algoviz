...
(() => {
  ...
  function renderGraph(snapshot) {
    ...
    if (snapshot) {
      ...
      if (
        snapshot.action === "cycle-detected" &&
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      if (
        snapshot &&
        nodeState[edges[i].from] === "completed" &&
        nodeState[edges[i].to] === "completed"
      ) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (
        snapshot &&
        snapshot.inDegrees &&
        snapshot.inDegrees[nodes[i]] !== undefined
      ) {
  ...
  function renderInDegrees(snapshot) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      const isCycle =
        snapshot.action === "cycle-detected" && !inOrder && deg > 0;
  ...
  function renderCurrentStep() {
    const snapshot =
      currentStep >= 0 && currentStep < snapshots.length
    ...
    if (snapshot) {
      ...
      switch (snapshot.action) {
        ...
        case "dequeue":
          actionText =
            "Dequeued " + snapshot.currentNode + " — added to sorted order.";
          break;
        case "update-neighbors":
          actionText =
            "Updated neighbors of " +
            snapshot.currentNode +
            " — decremented in-degrees.";
          break;
        case "cycle-detected":
  ...
  function addNode() {
    ...
    updateStats();
    infoEl.textContent =
      "Added node '" + name + "'. Total: " + nodes.length + " nodes.";
  }
  ...
  function loadPreset() {
    ...
    edges = preset.edges.map(function (e) {
  ...
  function runAlgorithm() {
    ...
    if (result.hasCycle) {
      infoEl.textContent =
        "Cycle detected! " +
        result.cycleNodes.length +
        " node(s) are stuck. Use step controls to see the algorithm progress.";
    } else {
      infoEl.textContent =
        "Algorithm ready. Order: " +
        result.order.join(" \u2192 ") +
